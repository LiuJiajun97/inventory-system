...
export const stockApi = {
  ...
  // V26 库存冻结/解冻(仓+批次粒度,只拦出不拦入)
  freeze: (data: { warehouseId: number; batchNo: string; reason: string }) =>
    http.post<unknown, void>("/stock/freeze", data),
  unfreeze: (data: { warehouseId: number; batchNo: string }) =>
    http.post<unknown, void>("/stock/unfreeze", data),
  freezeLogs: (params: {
    warehouseId?: number;
    batchNo?: string;
    page?: number;
    pageSize?: number;
  }) => http.get<unknown, PagedResponse<FreezeLogRow>>("/stock/freeze-logs", { params }),
};

// V26 价目表(读跟随登录,写绑 price:edit)
export const priceApi = {
  list: (params: {
    ...
    page?: number;
    pageSize?: number;
  }) => http.get<unknown, PagedResponse<PriceList>>("/price-lists", { params }),
  get: (id: number) => http.get<unknown, PriceListDetail>(`/price-lists/${id}`),
  create: (data: {
    ownerType: string;
    ownerId: number;
    name?: string;
    ...
    lines: { itemId: number; unitPrice: number; taxRate?: number | null }[];
  }) => http.post<unknown, PriceList>("/price-lists", data),
  update: (
    id: number,
    data: {
      ownerType: string;
  ...
  ) => http.put<unknown, PriceList>(`/price-lists/${id}`, data),
  delete: (id: number) => http.delete<unknown, void>(`/price-lists/${id}`),
  effective: (params: { ownerType: string; ownerId: number; date?: string }) =>
    http.get<unknown, EffectivePrice[]>("/price-lists/effective", { params }),
};
...
export interface SalesQuotationCreatePayload {
  docDate: string;
  customerId: number;
  salespersonId?: number;
  warehouseId: number;
...
export const quotationApi = {
  list: (params?: {
    ...
    page?: number;
    pageSize?: number;
  }) => http.get<unknown, PagedResponse<SalesQuotation>>("/sales-quotations", { params }),
  get: (id: number) => http.get<unknown, SalesQuotation>(`/sales-quotations/${id}`),
  create: (data: SalesQuotationCreatePayload) =>
    http.post<unknown, SalesQuotation>("/sales-quotations", data),
  update: (id: number, data: SalesQuotationCreatePayload) =>
    http.put<unknown, SalesQuotation>(`/sales-quotations/${id}`, data),
  send: (id: number) => http.post<unknown, SalesQuotation>(`/sales-quotations/${id}/send`),
  voidDoc: (id: number) => http.post<unknown, SalesQuotation>(`/sales-quotations/${id}/void`),
};
...
export const requisitionApi = {
  list: (params?: {
    ...
    http.get<unknown, PagedResponse<PurchaseRequisition>>("/purchase-requisitions", {
      params,
    }),
  get: (id: number) => http.get<unknown, PurchaseRequisition>(`/purchase-requisitions/${id}`),
  create: (data: PurchaseRequisitionCreatePayload) =>
    http.post<unknown, PurchaseRequisition>("/purchase-requisitions", data),
  update: (id: number, data: PurchaseRequisitionCreatePayload) =>
    http.put<unknown, PurchaseRequisition>(`/purchase-requisitions/${id}`, data),
  submit: (id: number) =>
    http.post<unknown, PurchaseRequisition>(`/purchase-requisitions/${id}/submit`),
  cancel: (id: number) =>
    http.post<unknown, PurchaseRequisition>(`/purchase-requisitions/${id}/cancel`),
};
...
export const messageApi = {
  list: (params?: {
    ...
    page?: number;
    pageSize?: number;
  }) => http.get<unknown, PagedResponse<SysMessage>>("/messages", { params }),
  unreadCount: () =>
    http.get<unknown, { count: number }>("/messages/unread-count", { silent: true }),
  markRead: (id?: number) =>
    http.put<unknown, { count: number }>("/messages/read", id == null ? {} : { id }),
  // dashboard 挂载调用(预警 0 项时返回 200 + null,silent 由调用方决定)
  dailyAlert: (opts?: { silent?: boolean }) =>
    http.post<unknown, SysMessage | null>("/messages/daily-alert", undefined, {
      silent: opts?.silent,
    }),
};
