...
// 右侧 当前日期 + 授权仓徽标(非 admin)+ 角色 Tag + 铃铛(V25)+ 头像下拉(退出登录)
...
// 铃铛(V25):进主布局时拉一次未读数 + 最近 20 条,路由切换时刷新一次(无轮询);
// 点击条目 → 标记已读 + 跳 refDocLink 对应单据;无消息时 Empty,底部"全部已读"
...
import type { SysMessage } from "../types/phase1";

// 消息类型 Tag(V25:approval/conversion/alert_daily,纯前端静态映射,字典不收)
const MSG_TYPE_TAG: Record<string, { text: string; color: string }> = {
  approval: { text: "审批", color: "blue" },
  conversion: { text: "转换", color: "green" },
  alert_daily: { text: "预警", color: "orange" },
};

/** 相对时间:1 分钟内"刚刚",否则按分钟/小时/天分段,超 7 天落日期。 */
...
function MainLayoutInner() {
  ...
  // V25 站内消息:未读数 + 最近 20 条;不进 polling(挂载 + 路由切换时刷一次)
  const [unreadCount, setUnreadCount] = useState(0);
  const [messages, setMessages] = useState<SysMessage[]>([]);
  const refreshBell = useCallback(() => {
    // 未读数 silent,失败静默;列表同步加载,失败也静默
    messageApi.unreadCount().then((r) => setUnreadCount(r.count)).catch(() => undefined);
    messageApi
      .list({ page: 1, pageSize: 20 })
      .then((r) => setMessages(r.rows ?? []))
      .catch(() => undefined);
  }, []);
  useEffect(() => {
    refreshBell();
  }, [refreshBell, location.pathname]);
  // 点击消息:未读则标已读;带 refDocType 则跳对应单据详情
  const onMessageClick = async (m: SysMessage) => {
    if (!m.read) {
      try {
    ...
    const href = refDocHref(m.refDocType, m.refDocId, null);
    if (href) navigate(href);
  };
  // 全部已读:无 id 调 read(后端按 receiver 清零),本地状态同步翻 read 标志
      ...
      <Layout>
        <Header className="app-header">
          ...
          <div className="app-header-right">
            ...
            {/* V25 站内消息铃铛:未读 Badge + Dropdown 最近 20 条;无轮询,挂载/路由切换刷新 */}
            <Dropdown
              trigger={["click"]}
                          ...
                          >
                            ...
                            <span
                              style={{
                                ...
                                height: 8,
                                borderRadius: "50%",
                                background: m.read ? "transparent" : "#dc2626",
                                marginTop: 6,
                              }}
                            />
