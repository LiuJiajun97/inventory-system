...
import { itemApi, purchaseApi, priceApi, supplierApi, userApi } from "../../api";
...
export function PurchaseOrderNewPage() {
  ...
  // V26 价目表带价:对方单位切换时拉一次生效价目存 map({itemId: {unitPrice, taxRate}});
  // 选物品时价目命中优先于物品参考价预填(仅预填,可改);对方未选时不拉(空 map 回退现有行为)
  const [priceMap, setPriceMap] = useState<Record<number, { unitPrice: number; taxRate?: number }>>({});

  // V26:拉指定供应商的生效价目(日期取表头下单日期,未选默认今天)
  const loadSupplierPriceMap = (supplierId: number) => {
    const date = (headerForm.getFieldValue("docDate") as Dayjs | undefined)?.format("YYYY-MM-DD");
    priceApi
      .effective({ ownerType: "supplier", ownerId: supplierId, date: date || undefined })
      .then((rows) =>
        setPriceMap(
          Object.fromEntries(
            rows.map((r) => [
  ...
  useEffect(() => {
    ...
    purchaseApi
      ...
      .then((doc) => {
        ...
        // V26:编辑态也拉价目表(新增行选物品时可命中预填)
        loadSupplierPriceMap(doc.supplierId);
      })
      .catch(() => undefined);
  ...
  const handleLinesChange = (values: readonly LineRow[]) => {
    const merged: LineRow[] = values.map((row, idx) => {
      ...
      if (row.itemId != null) {
        ...
        // V26 联动 0:价目表命中 → 不含税单价/税率预填(价目优先于物品参考价;仅预填,用户可改)
        const hit = priceMap[row.itemId];
        if (hit) {
          if (next.unitPrice == null) next.unitPrice = hit.unitPrice;
          if (next.taxRate == null && hit.taxRate != null) next.taxRate = hit.taxRate;
        }
      ...
      <ProCard bodyStyle={{ padding: 0 }}>
        ...
        >
          ...
          <ProFormSelect
            ...
            fieldProps={{
              optionFilterProp: "label",
              // V26:对方切换时拉一次生效价目存 map;清空时置空回退物品参考价
              onChange: (v: number | undefined) => {
                if (v == null) {
                  setPriceMap({});
                  return;
                }
                loadSupplierPriceMap(v);
              },
            }}
          />
          <ProFormSelect
