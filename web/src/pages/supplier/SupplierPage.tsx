...
export function SupplierPage() {
  ...
  const canImport = hasPerm("supplier:import");
  const canExport = hasPerm("supplier:export");
  ...
  return (
    <>
      <ProTable<Supplier>
        rowKey="id"
        locale={{ emptyText: <EmptyHint text="当前筛选条件下暂无供应商" /> }}
        actionRef={actionRef}
        columns={[
          {
            title: "关键字",
          ...
          optionRender: (_searchConfig, _props, dom) => [
            ...
            canImport && (
              <ImportButton
                ...
                templateFilename="供应商导入模板.xlsx"
                onDone={() => actionRef.current?.reload()}
              />
            ),
            canExport && (
              <ExportButton key="export" url={exportUrl} filename="供应商.xlsx" />
            ),
            canCreate && (
