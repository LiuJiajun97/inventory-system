// 新建销售退货单(V11):create 即过账,提交同事务生成入库单加库存
// 布局与采购/销售新建统一:页面头部 + 全宽卡片 + 底部固定操作条
// 步骤:选原销售单(approved/completed/closed,仓库默认带出订单仓可改) →
// 拉原单行(单价/税率只读锁原价) → 每行填退货数量(上限=已发货-已退,超了红字)
// 批次/保质期字段照现有入库行规则(启用保质期仓库必填批次)
...
export function SalesReturnNewPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      salesApi.list({ status: "completed", page: 1, pageSize: 200 }),
      salesApi.list({ status: "closed", page: 1, pageSize: 200 }),
    ])
      .then(([a, b, c]) => {
  ...
  // 选原销售单:仓库默认带出订单仓(可改);拉详情生成行模板(可退量 = 已发货 - 已退)
  const onOrderChange = async (id?: number) => {
    ...
    try {
      ...
      const next: Row[] = (order.items ?? [])
        .map((it, i) => {
          const shipped = Number(it.shippedQty);
          ...
          return {
            ...
            returnableQty: shipped - returned,
          };
        })
        .filter((r) => r.returnableQty > 0);
      if (next.length === 0) {
        message.warning("该销售单已无可退数量");
      }
      setRows(next);
    } catch {
      // 拦截器已提示
  ...
  const onSubmit = async () => {
    if (!orderId) {
      message.warning("请先选择原销售订单");
      return;
    }
    if (!warehouseId) {
      message.warning("请选择退货入库仓库");
      return;
    }
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      if (currentWarehouse?.enableExpiry && !r.batchNo) {
        message.warning(`第 ${i + 1} 行:启用保质期的仓库必须指定批次`);
        return;
      }
    ...
    try {
      ...
      navigate("/sales-returns?refresh=" + Date.now());
    } catch {
      // 拦截器已处理
    } finally {
      setSubmitting(false);
  ...
  const columns: ColumnsType<Row> = [
    ...
    {
      // 批次:启用批次/保质期的仓库展示(保质期仓库必填,留空按现有入库规则处理)
      title: "批次号",
      width: 180,
      render: (_v, r) =>
        currentWarehouse?.enableBatch || currentWarehouse?.enableExpiry ? (
          <Input
            placeholder={
        ...
        <div className="doc-form-body">
          <Form form={form} layout="vertical">
            <Row gutter={16}>
              ...
              <Col span={6}>
                <Form.Item label="退货入库仓库" required>
                  <Select
                    placeholder="选择仓库"
                    value={warehouseId}
          ...
          <Table
            ...
            size="small"
            scroll={{ x: 1700 }}
            locale={{ emptyText: orderId ? "该销售单已无可退数量" : "选择原销售订单后自动生成行" }}
          />
        </div>
