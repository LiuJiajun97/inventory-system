...
export function SalesReturnListPage() {
  ...
  const openLineDetail = async (r: DocLine) => {
    try {
      setDetail(await salesReturnApi.get(r.docId));
    } catch {
      // 拦截器已提示
    }
  };
  ...
  const columns: ProColumns<SalesReturn>[] = [
    {
      ...
      render: (_v, r) => (
        <a style={{ fontFamily: "monospace", fontSize: 13 }} onClick={() => navigate("/sales-returns/new/" + r.id)}>
          {r.docNo}
        </a>
      ),
    },
  ...
  const {
    ...
    viewMode === "line" ? (lineColumns as unknown as ProColumns<SalesReturn>[]) : columns,
    "sales-return",
  );
  return (
    <>
