...
import { customerApi, itemApi, priceApi, salesApi, userApi, warehouseApi } from "../../api";
...
export function SalesOrderNewPage() {
  ...
  // V26 价目表带价:客户切换时拉一次生效价目存 map({itemId: {unitPrice, taxRate}});
  // 选物品时价目命中优先于物品参考销售价预填(仅预填,可改);客户未选时不拉(空 map 回退现有行为)
  const [priceMap, setPriceMap] = useState<Record<number, { unitPrice: number; taxRate?: number }>>({});

  // V26:拉指定客户的生效价目(日期取表头开单日期,未选默认今天)
  const loadCustomerPriceMap = (customerId: number) => {
    const date = (form.getFieldValue("docDate") as Dayjs | undefined)?.format("YYYY-MM-DD");
    priceApi
      .effective({ ownerType: "customer", ownerId: customerId, date: date || undefined })
      .then((rows) =>
        setPriceMap(
          Object.fromEntries(
            rows.map((r) => [
  ...
  useEffect(() => {
    ...
    salesApi
      ...
      .then((doc) => {
        ...
        loadCustomerPriceMap(doc.customerId);
      })
      .catch(() => undefined);
  ...
  const onItemChange = (key: number, itemId: number) => {
    ...
    setLines((ls) =>
      ls.map((l) => {
        ...
        // V26:选物品且单价未填且价目表命中 → 预填(价目优先于物品参考销售价;仅预填,用户可改)
        const hit = priceMap[itemId];
        const unitPrice =
          l.unitPrice == null && hit
            ? hit.unitPrice
            : l.unitPrice == null && it?.referenceSalePrice != null
              ? Number(it.referenceSalePrice)
              : l.unitPrice;
        // V23.1:预填了不含税 → 按带出的税率联动含税(与手动填不含税同口径);
        // V26:价目命中且带税率 → 优先于物品默认税率
        const taxRate =
          hit?.taxRate != null
            ? hit.taxRate
            : it
              ? Number(it.defaultTaxRate ?? 13)
              : l.taxRate;
        const taxPrice =
          unitPrice != null
