...
CREATE TABLE public.stock (
    ...
    frozen            boolean DEFAULT false NOT NULL       -- 冻结位(V26,仓+批次粒度)
);
...
COMMENT ON COLUMN public.stock.frozen IS '冻结位(V26):true=该仓该批次禁止出库/预占,冻结只拦出不拦入';
...
-- ============================================================
-- 表:stock_freeze_log 库存冻结审计流水
-- 说明:纯追加,记录每次冻结/解冻(V26),查询热路径不碰。
-- ============================================================
CREATE TABLE public.stock_freeze_log (
    id           serial NOT NULL                                    ,  -- 主键ID
    warehouse_id integer NOT NULL                                   ,  -- 仓库 ID
    batch_id     integer NOT NULL                                   ,  -- 批次 ID
    item_id      integer NOT NULL                                   ,  -- 物品 ID(回填展示用)
    action       text NOT NULL                                      ,  -- 操作:freeze 冻结 / unfreeze 解冻
    reason       text                                               ,  -- 原因(冻结必填,解冻可空)
    operator     text                                               ,  -- 操作人
    created_at   timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL  -- 创建时间
);

COMMENT ON TABLE public.stock_freeze_log IS '库存冻结审计流水(V26):纯追加,freeze/unfreeze 各一条,查询热路径不碰';

COMMENT ON COLUMN public.stock_freeze_log.id IS '主键ID';
COMMENT ON COLUMN public.stock_freeze_log.warehouse_id IS '仓库 ID';
COMMENT ON COLUMN public.stock_freeze_log.batch_id IS '批次 ID';
COMMENT ON COLUMN public.stock_freeze_log.item_id IS '物品 ID(回填展示用)';
COMMENT ON COLUMN public.stock_freeze_log.action IS '操作:freeze 冻结 / unfreeze 解冻';
COMMENT ON COLUMN public.stock_freeze_log.reason IS '原因(冻结必填,解冻可空)';
COMMENT ON COLUMN public.stock_freeze_log.operator IS '操作人';
COMMENT ON COLUMN public.stock_freeze_log.created_at IS '创建时间';

ALTER TABLE ONLY public.stock_freeze_log ADD CONSTRAINT stock_freeze_log_pkey PRIMARY KEY (id);

CREATE INDEX idx_stock_freeze_log_wh_batch ON public.stock_freeze_log USING btree (warehouse_id, batch_id, id);


-- ============================================================
-- 表:price_list 价目表
-- 说明:供应商/客户维度的生效价目(V26),命中规则=owner+物品+日期在
-- [valid_from, valid_until] 内(边界任一可空=不限)且 status=1。
-- ============================================================
CREATE TABLE public.price_list (
    id           serial NOT NULL                    ,  -- 主键ID
    owner_type   text NOT NULL                      ,  -- 对方类型:supplier / customer
    owner_id     integer NOT NULL                   ,  -- 对方 ID(供应商/客户表主键)
    name         text                               ,  -- 价目名称(可空,默认对方单位简称)
    valid_from   date                               ,  -- 生效起(可空=不限)
    valid_until  date                               ,  -- 生效止(可空=不限)
...
COMMENT ON TABLE public.price_list IS '价目表(V26):供应商/客户维度的生效价目,命中规则=owner+物品+日期在 [valid_from, valid_until] 内且 status=1';

COMMENT ON COLUMN public.price_list.id IS '主键ID';
COMMENT ON COLUMN public.price_list.owner_type IS '对方类型:supplier 供应商 / customer 客户';
COMMENT ON COLUMN public.price_list.owner_id IS '对方 ID(供应商/客户表主键)';
COMMENT ON COLUMN public.price_list.name IS '价目名称(可空,默认对方单位简称)';
COMMENT ON COLUMN public.price_list.valid_from IS '生效起(可空=不限)';
COMMENT ON COLUMN public.price_list.valid_until IS '生效止(可空=不限)';
COMMENT ON COLUMN public.price_list.status IS '状态:1 启用 / 0 停用(停用不参与命中)';
COMMENT ON COLUMN public.price_list.creator IS '创建人';
COMMENT ON COLUMN public.price_list.created_at IS '创建时间';
COMMENT ON COLUMN public.price_list.updater IS '更新人';
COMMENT ON COLUMN public.price_list.updated_at IS '更新时间';

ALTER TABLE ONLY public.price_list ADD CONSTRAINT price_list_pkey PRIMARY KEY (id);

CREATE INDEX idx_price_list_owner ON public.price_list USING btree (owner_type, owner_id, valid_from);


-- ============================================================
-- 表:price_list_item 价目表行
-- 说明:物品不含税单价 + 税率可空(NULL=按物品默认税率),唯一键 (price_list_id, item_id)。
-- ============================================================
CREATE TABLE public.price_list_item (
    id            serial NOT NULL                    ,  -- 主键ID
    price_list_id integer NOT NULL                   ,  -- 价目表 ID
    item_id       integer NOT NULL                   ,  -- 物品 ID
    unit_price    numeric(18,4) NOT NULL             ,  -- 不含税单价
    tax_rate      numeric(5,2)                       ,  -- 税率(百分数,可空=NULL 按物品默认税率)
    creator       text                                               -- 创建人
);

COMMENT ON TABLE public.price_list_item IS '价目表行(V26):物品不含税单价,税率可空=NULL 按物品默认税率';

COMMENT ON COLUMN public.price_list_item.id IS '主键ID';
COMMENT ON COLUMN public.price_list_item.price_list_id IS '价目表 ID';
COMMENT ON COLUMN public.price_list_item.item_id IS '物品 ID';
COMMENT ON COLUMN public.price_list_item.unit_price IS '不含税单价';
COMMENT ON COLUMN public.price_list_item.tax_rate IS '税率(百分数,可空=NULL 按物品默认税率)';
COMMENT ON COLUMN public.price_list_item.creator IS '创建人';

ALTER TABLE ONLY public.price_list_item ADD CONSTRAINT price_list_item_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.price_list_item ADD CONSTRAINT price_list_item_list_item_key UNIQUE (price_list_id, item_id);
...
CREATE TABLE public.purchase_order (
    ...
    ref_doc_type            text                               ,  -- 关联源单类型:V25 requisition(请购单)
    ref_doc_no              text                               ,  -- 关联源单单号
    ref_doc_id              integer                              -- 关联源单 ID
);
...
COMMENT ON COLUMN public.purchase_order.ref_doc_type IS '关联源单类型:V25 requisition(请购单,本系统内唯一来源)';
COMMENT ON COLUMN public.purchase_order.ref_doc_no IS '关联源单单号';
COMMENT ON COLUMN public.purchase_order.ref_doc_id IS '关联源单 ID';
...
CREATE TABLE public.sales_order (
    ...
    currency_code       character varying(3)               ,  -- 币种(如 CNY)
    exchange_rate       numeric(14,6)                      ,  -- 汇率
    ref_doc_type        text                               ,  -- 关联源单类型:V25 quotation(销售报价单)
    ref_doc_no          text                               ,  -- 关联源单单号
    ref_doc_id          integer                              -- 关联源单 ID
);
...
COMMENT ON COLUMN public.sales_order.ref_doc_type IS '关联源单类型:V25 quotation(销售报价单,本系统内唯一来源)';
COMMENT ON COLUMN public.sales_order.ref_doc_no IS '关联源单单号';
COMMENT ON COLUMN public.sales_order.ref_doc_id IS '关联源单 ID';
...
COMMENT ON COLUMN public.dict.dict_type IS '字典类型:warehouseType / itemCategory / settleMethod / dept';
...
-- ============================================================
-- 表:sales_quotation 销售报价单
-- 说明:V25 销售报价单(表 sales_quotation):状态 draft/sent/converted/voided,无审批。
-- ============================================================
CREATE TABLE public.sales_quotation (
    id                   serial NOT NULL                    ,  -- 主键ID
    doc_no               text NOT NULL                      ,  -- 单据编号(BJ-YYYYMMDD-NNNN,唯一)
    doc_date             date NOT NULL                      ,  -- 单据日期
    customer_id          integer NOT NULL                   ,  -- 客户 ID
    salesperson_id       integer                              ,  -- 销售员用户 ID(可空)
    warehouse_id         integer NOT NULL                   ,  -- 发货仓库 ID
    total_amount         numeric(18,4) DEFAULT 0 NOT NULL   ,  -- 整单不含税合计
    total_tax_amount     numeric(18,4) DEFAULT 0 NOT NULL   ,  -- 整单税额合计
    total_tax_inclusive  numeric(18,4) DEFAULT 0 NOT NULL   ,  -- 整单价税合计
    quote_valid_until    date                                 ,  -- 报价有效期(可空)
    status               text DEFAULT 'draft'::text NOT NULL,  -- 单据状态:draft/sent/converted/voided
    remark               text                                 ,  -- 备注
    creator              text                                 ,  -- 制单人
    created_at           timestamp without time zone          ,  -- 制单时间
    updater              text                                 ,  -- 更新人
...
COMMENT ON TABLE public.sales_quotation IS '销售报价单(V25):状态 draft/sent/converted/voided,无审批';

COMMENT ON COLUMN public.sales_quotation.id IS '主键ID';
COMMENT ON COLUMN public.sales_quotation.doc_no IS '单据编号(BJ-YYYYMMDD-NNNN,唯一)';
COMMENT ON COLUMN public.sales_quotation.doc_date IS '单据日期';
COMMENT ON COLUMN public.sales_quotation.customer_id IS '客户 ID';
COMMENT ON COLUMN public.sales_quotation.salesperson_id IS '销售员用户 ID(可空)';
COMMENT ON COLUMN public.sales_quotation.warehouse_id IS '发货仓库 ID';
COMMENT ON COLUMN public.sales_quotation.total_amount IS '整单不含税合计';
COMMENT ON COLUMN public.sales_quotation.total_tax_amount IS '整单税额合计';
COMMENT ON COLUMN public.sales_quotation.total_tax_inclusive IS '整单价税合计';
COMMENT ON COLUMN public.sales_quotation.quote_valid_until IS '报价有效期(可空,过期仅展示标记)';
COMMENT ON COLUMN public.sales_quotation.status IS '单据状态:draft/sent/converted/voided';
COMMENT ON COLUMN public.sales_quotation.remark IS '备注';
COMMENT ON COLUMN public.sales_quotation.creator IS '制单人';
COMMENT ON COLUMN public.sales_quotation.created_at IS '制单时间';
COMMENT ON COLUMN public.sales_quotation.updater IS '更新人';
COMMENT ON COLUMN public.sales_quotation.updated_at IS '更新时间';

ALTER TABLE ONLY public.sales_quotation ADD CONSTRAINT sales_quotation_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.sales_quotation ADD CONSTRAINT sales_quotation_doc_no_key UNIQUE (doc_no);


-- ============================================================
-- 表:sales_quotation_item 销售报价单明细行
-- 说明:V25 销售报价单行(表 sales_quotation_item),六列金额照抄销售订单行。
-- ============================================================
CREATE TABLE public.sales_quotation_item (
    id                   serial NOT NULL                    ,  -- 主键ID
    quotation_id         integer NOT NULL                   ,  -- 报价单 ID
    line_no              integer NOT NULL                   ,  -- 行号
    item_id              integer NOT NULL                   ,  -- 物品 ID
    quantity             numeric(18,4) NOT NULL             ,  -- 数量
    unit_price           numeric(18,4)                       ,  -- 不含税单价(可空)
    tax_price            numeric(18,4)                       ,  -- 含税单价(可空)
    tax_rate             numeric(5,2) DEFAULT 13.00 NOT NULL,  -- 税率(百分数)
    amount               numeric(18,4)                       ,  -- 不含税金额(服务端重算)
    tax_amount           numeric(18,4)                       ,  -- 税额(服务端重算)
    total_amount         numeric(18,4)                       ,  -- 价税合计(服务端重算)
    remark               text                                 ,  -- 行备注
    creator              text                                  -- 创建人
);
...
ALTER TABLE ONLY public.sales_quotation_item ADD CONSTRAINT sales_quotation_item_pkey PRIMARY KEY (id);


-- ============================================================
-- 表:purchase_requisition 请购单
-- 说明:V25 请购单(表 purchase_requisition):状态 draft/submitted/converted/cancelled,无审批。
-- ============================================================
CREATE TABLE public.purchase_requisition (
    id                   serial NOT NULL                    ,  -- 主键ID
    doc_no               text NOT NULL                      ,  -- 单据编号(QG-YYYYMMDD-NNNN,唯一)
    doc_date             date NOT NULL                      ,  -- 单据日期
    warehouse_id         integer NOT NULL                   ,  -- 收货仓库 ID
    applicant_id         integer NOT NULL                   ,  -- 申请人用户 ID
    department           text                                 ,  -- 申请部门(字典 dept 类型的 dict_key,可空)
    status               text DEFAULT 'draft'::text NOT NULL,  -- 单据状态:draft/submitted/converted/cancelled
    remark               text                                 ,  -- 备注
    creator              text                                 ,  -- 制单人
    created_at           timestamp without time zone          ,  -- 制单时间
    updater              text                                 ,  -- 更新人
...
COMMENT ON TABLE public.purchase_requisition IS '请购单(V25):状态 draft/submitted/converted/cancelled,无审批';

COMMENT ON COLUMN public.purchase_requisition.id IS '主键ID';
COMMENT ON COLUMN public.purchase_requisition.doc_no IS '单据编号(QG-YYYYMMDD-NNNN,唯一)';
COMMENT ON COLUMN public.purchase_requisition.doc_date IS '单据日期';
COMMENT ON COLUMN public.purchase_requisition.warehouse_id IS '收货仓库 ID';
COMMENT ON COLUMN public.purchase_requisition.applicant_id IS '申请人用户 ID';
COMMENT ON COLUMN public.purchase_requisition.department IS '申请部门(字典 dept 类型的 dict_key,可空)';
COMMENT ON COLUMN public.purchase_requisition.status IS '单据状态:draft/submitted/converted/cancelled';
COMMENT ON COLUMN public.purchase_requisition.remark IS '备注';
COMMENT ON COLUMN public.purchase_requisition.creator IS '制单人';
COMMENT ON COLUMN public.purchase_requisition.created_at IS '制单时间';
COMMENT ON COLUMN public.purchase_requisition.updater IS '更新人';
COMMENT ON COLUMN public.purchase_requisition.updated_at IS '更新时间';

ALTER TABLE ONLY public.purchase_requisition ADD CONSTRAINT purchase_requisition_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.purchase_requisition ADD CONSTRAINT purchase_requisition_doc_no_key UNIQUE (doc_no);


-- ============================================================
-- 表:purchase_requisition_item 请购单明细行
-- 说明:V25 请购单行(表 purchase_requisition_item),无金额合计(行价可空不汇总)。
-- ============================================================
CREATE TABLE public.purchase_requisition_item (
    id                   serial NOT NULL                    ,  -- 主键ID
    requisition_id       integer NOT NULL                   ,  -- 请购单 ID
    line_no              integer NOT NULL                   ,  -- 行号
    item_id              integer NOT NULL                   ,  -- 物品 ID
    quantity             numeric(18,4) NOT NULL             ,  -- 数量
    expected_date        date                                 ,  -- 期望到货日(可空)
    unit_price           numeric(18,4)                       ,  -- 参考单价(可空)
    remark               text                                 ,  -- 行备注
    creator              text                                  -- 创建人
);
...
COMMENT ON COLUMN public.purchase_requisition_item.unit_price IS '参考单价(可空,仅参考)';
...
ALTER TABLE ONLY public.purchase_requisition_item ADD CONSTRAINT purchase_requisition_item_pkey PRIMARY KEY (id);


-- ============================================================
-- 表:sys_message 站内消息
-- 说明:V25 站内消息(表 sys_message):审批/转换/预警通知。
-- ============================================================
CREATE TABLE public.sys_message (
    id                   serial NOT NULL                    ,  -- 主键ID
    receiver_id          integer NOT NULL                   ,  -- 接收人用户 ID
    type                 text NOT NULL                      ,  -- 消息类型:approval/conversion/alert_daily
    title                text NOT NULL                      ,  -- 消息标题
    content              text NOT NULL                      ,  -- 消息内容
    ref_doc_type         text                                 ,  -- 关联单据类型(可空)
    ref_doc_id           integer                               ,  -- 关联单据 ID(可空)
    is_read              boolean DEFAULT false NOT NULL     ,  -- 是否已读
    created_at           timestamp without time zone NOT NULL  -- 创建时间
);

COMMENT ON TABLE public.sys_message IS '站内消息(V25):审批/转换/预警通知';

COMMENT ON COLUMN public.sys_message.id IS '主键ID';
COMMENT ON COLUMN public.sys_message.receiver_id IS '接收人用户 ID';
COMMENT ON COLUMN public.sys_message.type IS '消息类型:approval/conversion/alert_daily';
COMMENT ON COLUMN public.sys_message.title IS '消息标题';
COMMENT ON COLUMN public.sys_message.content IS '消息内容';
COMMENT ON COLUMN public.sys_message.ref_doc_type IS '关联单据类型(可空,如 quotation/requisition)';
COMMENT ON COLUMN public.sys_message.ref_doc_id IS '关联单据 ID(可空)';
COMMENT ON COLUMN public.sys_message.is_read IS '是否已读';
COMMENT ON COLUMN public.sys_message.created_at IS '创建时间';

ALTER TABLE ONLY public.sys_message ADD CONSTRAINT sys_message_pkey PRIMARY KEY (id);
CREATE INDEX idx_sys_message_receiver_read ON public.sys_message USING btree (receiver_id, is_read);
