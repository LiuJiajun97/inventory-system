...
/**
 * 新建销售退货单入参(create 即过账,服务端按原单行锁价并重算金额)。
 *
 * @param salesOrderId 原销售订单 ID(必须已审批)
...
public record SalesReturnCreateDTO(
        @NotNull(message = "原销售订单 ID 必填")
        @Positive(message = "原销售订单 ID 必须为正数") Long salesOrderId,
        @NotNull(message = "仓库 ID 必填")
        @Positive(message = "仓库 ID 必须为正数") Long warehouseId,
        LocalDate docDate,
        String remark,
        @Valid
        @NotEmpty(message = "至少 1 行") List<SalesReturnLineDTO> items) {
}
