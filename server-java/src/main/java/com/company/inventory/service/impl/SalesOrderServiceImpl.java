...
public class SalesOrderServiceImpl implements SalesOrderService {
     ...
     * @param salesQuotationMapper 销售报价单 Mapper(V25)
     * @param messageService       站内消息服务(V25)
     */
    public SalesOrderServiceImpl(SalesOrderMapper orderMapper, SalesOrderItemMapper itemMapper,
            ...
            SalesQuotationMapper salesQuotationMapper, MessageService messageService) {
    ...
    public SalesOrderVO create(SalesOrderCreateDTO dto, String username) {
        ...
        // V25:转换源单(销售报价单)校验 + 落库 + 标记 + 通知
        SalesQuotationDO quotation = null;
        if (dto.refDocType() != null || dto.refDocId() != null) {
            quotation = consumeQuotation(dto, order);
        }
        ...
        SalesOrderVO vo = get(order.getId());
        if (quotation != null) {
            notifyQuotationConverted(quotation, order, username);
        }
        return vo;
    }
    ...
    public PageResult<SalesOrderVO> list(SalesOrderQuery query) {
        // 数据权限:未授权用户查空;授权用户只看发货仓在授权仓内的销售订单(admin 豁免不过滤)
        ...
        if (allowed != null) {
            wrapper.in(SalesOrderDO::getWarehouseId, allowed);
        }
    ...
    /**
     * 查订单,不存在抛 404,非 admin 无权访问该发货仓单据抛 403。
     *
     * @param id 订单 ID
     * @return 订单
     */
    private SalesOrderDO requireOrder(long id) {
        ...
        assertWarehouseAccess(order.getWarehouseId());
        return order;
    }
    ...
    /**
     * 校验并消费销售报价单(V25 转换源单):仅 draft/sent、未过期、未被其他订单引用,
     * 通过后设置订单 ref 三列并返回报价单实体(后续置 converted + 发消息)。
     *
     * @param dto   订单入参
     * @param order 订单实体(本处未入库,仅设 ref 三列)
     * @return 报价单实体
     */
    private SalesQuotationDO consumeQuotation(SalesOrderCreateDTO dto, SalesOrderDO order) {
        if (!"quotation".equals(dto.refDocType())) {
            throw new BizException("销售订单关联源单类型仅支持 quotation");
        }
        if (dto.refDocId() == null) {
            throw new BizException("refDocId 必填");
        }
        SalesQuotationDO quotation = salesQuotationMapper.selectById(dto.refDocId());
        if (quotation == null) {
            throw new BizException("关联销售报价单不存在: id=" + dto.refDocId());
        }
        String status = quotation.getStatus();
        if (QuotationStatus.VOIDED.equals(status) || QuotationStatus.CONVERTED.equals(status)) {
            throw new BizException("报价单已作废/已转换,不可转换");
        }
        if (quotation.getQuoteValidUntil() != null
                && quotation.getQuoteValidUntil().isBefore(LocalDate.now())) {
            throw new BizException("报价单已过期,不可转换");
        }
        // 重复引用:该报价单已被其他订单引用 → 400
        SalesOrderDO other = orderMapper.selectOne(new LambdaQueryWrapper<SalesOrderDO>()
                .eq(SalesOrderDO::getRefDocType, "quotation")
                .eq(SalesOrderDO::getRefDocId, dto.refDocId())
                .ne(SalesOrderDO::getId, order.getId() == null ? -1L : order.getId())
                .last("LIMIT 1"));
        if (other != null) {
            throw new BizException("报价单已被订单 " + other.getDocNo() + " 引用,不可重复转换");
        }
        order.setRefDocType("quotation");
        order.setRefDocNo(quotation.getDocNo());
        order.setRefDocId(quotation.getId());
        // 标记源单 converted(条件更新,防并发重入)
        salesQuotationMapper.update(null, new com.baomidou.mybatisplus.core.conditions.update
                .UpdateWrapper<SalesQuotationDO>()
                .set("status", QuotationStatus.CONVERTED)
                .set("updater", order.getCreator() == null ? "system" : order.getCreator())
                .set("updated_at", LocalDateTime.now())
                .eq("id", quotation.getId())
                .in("status", List.of(QuotationStatus.DRAFT, QuotationStatus.SENT)));
        return quotation;
    }
     ...
     * 发送报价单转换通知(失败只打 warn)。
     *
     * @param quotation 源报价单
     * @param order     新订单
     * @param username  当前操作人
     */
    private void notifyQuotationConverted(SalesQuotationDO quotation, SalesOrderDO order,
            String username) {
        try {
            if (quotation.getCreator() == null) {
                return;
            }
            if (quotation.getCreator().equals(username)) {
                return;
            }
            MessageServiceImpl impl = (MessageServiceImpl) messageService;
            Long receiverId = impl.lookupUserId(quotation.getCreator());
            if (receiverId == null) {
                return;
            }
            messageService.trySendQuietly(receiverId, MessageType.CONVERSION,
                    "报价单已转销售订单",
                    "你的报价单 " + quotation.getDocNo() + " 已转换为销售订单 " + order.getDocNo(),
                    "quotation", quotation.getId());
        } catch (Exception e) {
            org.slf4j.LoggerFactory.getLogger(SalesOrderServiceImpl.class)
                    .warn("报价单转换通知失败: {}", e.getMessage());
        }
    }
     ...
     */
    private void notifyApprovalResult(SalesOrderDO order, String approver,
        ...
        try {
            ...
            String title = approved ? "你的销售订单已批准" : "你的销售订单已驳回";
            String content = approved
                    ? "你的销售订单 " + order.getDocNo() + " 已批准"
                    : "你的销售订单 " + order.getDocNo() + " 已驳回" + (reason == null ? "" : ",原因:" + reason);
            messageService.trySendQuietly(receiverId, MessageType.APPROVAL,
                    title, content, "sales_order", order.getId());
        } catch (Exception e) {
            org.slf4j.LoggerFactory.getLogger(SalesOrderServiceImpl.class)
                    .warn("销售订单审批通知失败: {}", e.getMessage());
        }
    }
}
