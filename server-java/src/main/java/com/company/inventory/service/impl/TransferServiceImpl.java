...
public class TransferServiceImpl implements TransferService {
    ...
    public TransferDocVO create(TransferCreateDTO dto, String username) {
        ...
        requireSameWarehouseType(dto.fromWarehouseId(), dto.toWarehouseId());
    ...
    public TransferDocVO reject(long id, TransferActionDTO dto, String username) {
        ...
        notifyApprovalResult(doc, username, false, dto.rejectReason());
        return get(id);
    }
    ...
    /**
     * 校验调拨源仓与目的仓类型一致(跨类型调拨如原材料仓→成品仓无业务意义,硬禁止)。
     *
     * @param fromWarehouseId 源仓 ID
     * @param toWarehouseId   目的仓 ID
     */
    private void requireSameWarehouseType(long fromWarehouseId, long toWarehouseId) {
        WarehouseDO from = warehouseMapper.selectById(fromWarehouseId);
        WarehouseDO to = warehouseMapper.selectById(toWarehouseId);
        if (!from.getWarehouseType().equals(to.getWarehouseType())) {
            throw new BizException("调拨源仓与目的仓类型不一致,不允许跨类型调拨");
        }
    }
     ...
     */
    private void notifyApprovalResult(TransferDocDO doc, String approver,
        ...
        try {
            ...
            String title = approved ? "你的调拨单已批准" : "你的调拨单已驳回";
            String content = approved
                    ? "你的调拨单 " + doc.getDocNo() + " 已批准"
                    : "你的调拨单 " + doc.getDocNo() + " 已驳回" + (reason == null ? "" : ",原因:" + reason);
            messageService.trySendQuietly(receiverId, MessageType.APPROVAL,
                    title, content, "transfer", doc.getId());
        } catch (Exception e) {
            org.slf4j.LoggerFactory.getLogger(TransferServiceImpl.class)
                    .warn("调拨单审批通知失败: {}", e.getMessage());
        }
    }
}
