...
public class StocktakeServiceImpl implements StocktakeService {
     ...
     */
    private void notifyApprovalResult(StocktakeDocDO doc, String approver,
        ...
        try {
            ...
            String title = approved ? "你的盘点单已批准" : "你的盘点单已驳回";
            String content = approved
                    ? "你的盘点单 " + doc.getDocNo() + " 已批准"
                    : "你的盘点单 " + doc.getDocNo() + " 已驳回" + (reason == null ? "" : ",原因:" + reason);
            messageService.trySendQuietly(receiverId, MessageType.APPROVAL,
                    title, content, "stocktake", doc.getId());
        } catch (Exception e) {
            org.slf4j.LoggerFactory.getLogger(StocktakeServiceImpl.class)
                    .warn("盘点单审批通知失败: {}", e.getMessage());
        }
    }
}
