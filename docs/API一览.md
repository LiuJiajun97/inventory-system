...
| POST | `/stock/freeze` `/stock/unfreeze` | 库存冻结(原因必填)/ 解冻(仓+批次粒度,只拦出不拦入,重复/未冻结 400) | admin+operator |
| GET | `/stock/freeze-logs` | 冻结审计流水(分页,带物品/批次回填) | 登录 |
| GET/POST/PUT/DELETE | `/price-lists` `/price-lists/:id` `/price-lists/effective` | 价目表(供应商/客户,行=物品+不含税单价+税率)/ 生效价按日期查 | 登录 / admin+operator |
...
| GET/POST/PUT | `/purchase-orders` `/purchase-orders/:id/...` | 采购订单 + 提交/审批/驳回/到货(PUT 编辑,仅草稿/已驳回;create 可带 ref 三字段由请购单转入) | 登录 / admin+operator |
| GET/POST/PUT | `/sales-orders` `/sales-orders/:id/...` | 销售订单 + 提交/审批/驳回/发货(PUT 编辑,仅草稿/已驳回;create 可带 ref 三字段由报价单转入) | 登录 / admin+operator |
| GET/POST/PUT | `/sales-quotations` `/sales-quotations/:id/...` | 销售报价单 + 发送/作废(无审批,PUT 编辑仅草稿;POST /:id/send 发送、/:id/void 作废;create 即 draft) | 登录 / admin+operator |
| GET/POST/PUT | `/purchase-requisitions` `/purchase-requisitions/:id/...` | 请购单 + 提交/取消(无审批,PUT 编辑仅草稿;申请部门存字典 dept 类型 dict_key) | 登录 / admin+operator |
...
| GET/PUT/POST/GET | `/messages` `/messages/read` `/messages/daily-alert` `/messages/unread-count`(V25) | 站内消息:列表(分页+unread 筛选)/单条或全部已读({id} 单条,空对象全读)/仪表盘每日预警汇总(当日幂等,预警 0 项不生成)/未读数 | 登录(只查自己的) |
...
- **可观测性端点**(`/actuator/**`,无需登录,单机轻量部署;生产需网关鉴权):`GET /actuator/health`(UP/DOWN)、`GET /actuator/info`、`GET /actuator/metrics`。
